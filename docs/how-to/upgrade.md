...
An `0.1.0-alpha.14` artifact prints:

```text
Database migration 0005_reconciliation_state_index is compatible.
```

The target artifact prints `0008_recovery_api_budget` after the
migration in step 5.
...
FIRST_MIGRATION_LOG="$CHANGE_RECORD_DIR/extra-codeowners-0008-first-migration.log"
...
```bash
SECOND_MIGRATION_LOG="$CHANGE_RECORD_DIR/extra-codeowners-0008-final-migration.log"
...
is already at `0008_recovery_api_budget`. Alembic makes no schema change, but
