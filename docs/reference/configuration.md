...
Only three query parameters are supported:
...
bypass the required SSL transport. They pin `search_path=public` instead of
accepting caller-supplied libpq `options`.
...
| `EXTRA_CODEOWNERS_AUTHORITY_INACCESSIBLE_RETRY_MAX_SECONDS` | integer | `21600` | Maximum delay for an authority `404` or `410` when current membership does not prove the repository is outside the installation; inclusive range `300` through `86400`. The first retry waits at least five minutes and later retries back off to this ceiling. A matching authority event coalesces the row and wakes it immediately. |
...
| `EXTRA_CODEOWNERS_WORKER_INVALIDATION_CONCURRENCY` | integer | `2` | Exact-head invalidation consumers per process; inclusive range `2` through `4`. One lane takes direct invalidations first and one takes recovery invalidations first. Each borrows the other class only when its own queue is empty. |
...
| `EXTRA_CODEOWNERS_WEBHOOK_DELIVERY_RETENTION_DAYS` | integer | `30` | Retain accepted GitHub delivery IDs for replay deduplication; inclusive range `1` through `3650` days. The elected reconciler prunes older IDs, unused finished head epochs, and stale reconciliation fingerprints on each run. |
...
- Production service startup requires an HTTPS GitHub API URL, including during the credential-free setup flow. Outside that flow it also requires complete GitHub credentials and a webhook secret of at least 32 bytes. Every non-local PostgreSQL connection must use `sslmode=require`; an operator-controlled loopback or Unix-socket transport is also allowed.
