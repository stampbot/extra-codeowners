...
| `installation_repositories` | Add repositories | Refresh just the added repository when the event identifies one ordinary target in the installation account with no removals. Adding the organization-policy repository, adding several repositories, or receiving ambiguous evidence still schedules installation-wide fan-out. |
